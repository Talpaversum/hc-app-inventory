...
export async function registerLocationRoutes(app: FastifyInstance) {
  app.get("/v1/locations", async (request, reply) => {
    ...
    const result = await pool.query(
      "select l.id, l.parent_id, l.name, lk.key as kind_key, lk.label as kind_label from locations l join location_kinds lk on lk.id = l.kind_id where l.tenant_id = $1 order by lk.sort_order, l.name",
      [tenantId],
    );
  ...
  app.post("/v1/locations", async (request, reply) => {
    ...
    const kind = await pool.query(
      "select id from location_kinds where tenant_id = $1 and key = $2",
    ...
    const result = await pool.query(
      "insert into locations (tenant_id, kind_id, name, parent_id) values ($1, $2, $3, $4) returning id",
  ...
  app.get("/v1/location-kinds", async (request, reply) => {
    ...
    const pool = getPool();
    const result = await pool.query(
      "select id, key, label, is_builtin from location_kinds where tenant_id = $1 order by sort_order",
      [tenantId],
    );
    return reply.send({ items: result.rows });
  });
  ...
  app.post("/v1/location-kinds", async (request, reply) => {
    ...
    const pool = getPool();
    const result = await pool.query(
      "insert into location_kinds (tenant_id, key, label, sort_order, is_builtin) values ($1, $2, $3, 100, false) returning id",
