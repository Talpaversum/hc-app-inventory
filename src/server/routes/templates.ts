...
export async function registerTemplateRoutes(app: FastifyInstance) {
  app.get("/v1/templates", async (request, reply) => {
    ...
    const result = await pool.query(
      "select id, name, visibility_scope, visibility_ref_id, is_locked from templates where tenant_id = $1 order by created_at",
      [tenantId],
    );
  ...
  app.post("/v1/templates", async (request, reply) => {
    ...
    const pool = getPool();
    const result = await pool.query(
      "insert into templates (tenant_id, name, visibility_scope, visibility_ref_id, is_locked) values ($1, $2, $3, $4, false) returning id",
    ...
    for (const field of body.fields ?? []) {
      await pool.query(
        "insert into template_fields (template_id, attribute_type_id, required, sort_order) values ($1, $2, $3, $4)",
