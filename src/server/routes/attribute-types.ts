...
export async function registerAttributeTypeRoutes(app: FastifyInstance) {
  app.get("/v1/attribute-types", async (request, reply) => {
    ...
    const result = await pool.query(
      "select id, key, label, data_type, is_builtin, validation_regex, unique_scope from attribute_types where tenant_id = $1 order by created_at",
      [tenantId],
    );
  ...
  app.post("/v1/attribute-types", async (request, reply) => {
    ...
    const pool = getPool();
    const result = await pool.query(
      "insert into attribute_types (tenant_id, key, label, data_type, validation_regex, unique_scope, is_builtin) values ($1, $2, $3, $4, $5, $6, false) returning id",
